...
class EthosUBackend final : public ::executorch::runtime::BackendInterface {
 public:
  ...
  Error execute(
    ...
    for (int i = 0; i < handles.outputs->count; i++) {
      ...
      ET_LOG(
          Error,
          "Output %d: tensor_bytes=%zu io_bytes=%zu tensor_count=%d io_count=%d elem_size=%d shape=[%d,%d,%d,%d,%d,%d] tensor_dim=%d offset=%d",
          i,
          tensor_bytes,
          io_bytes,
          tensor_count,
          io_count,
          handles.outputs->io[i].elem_size,
          handles.outputs->io[i].shape[0],
          handles.outputs->io[i].shape[1],
          handles.outputs->io[i].shape[2],
          handles.outputs->io[i].shape[3],
          handles.outputs->io[i].shape[4],
          handles.outputs->io[i].shape[5],
          tensor_out.dim(),
          handles.outputs->io[i].offset);

      if (tensor_bytes != io_bytes) {
        ET_LOG(
            Error,
            "Output %d: MISMATCH tensor_bytes(%zu) != io_bytes(%zu)",
            i,
            tensor_bytes,
            io_bytes);
        ...
        if (status != Error::Ok) {
          ET_LOG(
              Error,
              "Output %d: copy_with_layout_adjustment FAILED status=%d",
              i,
 ...
 private:
  ...
  Error copy_with_layout_adjustment(
    ...
    ET_LOG(
        Error,
        "copy_with_layout output %d: chunk_count=%zu vela_chunk=%zu chunk=%zu vela_elems=%zu elem_sz=%d",
        output_index,
        chunk_count,
        vela_chunk_size,
        chunk_size,
